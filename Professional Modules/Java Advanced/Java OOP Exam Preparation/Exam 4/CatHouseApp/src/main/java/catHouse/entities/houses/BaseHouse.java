...
public abstract class BaseHouse implements House {
    ...
    public String getStatistics() {
        return this.name + " " + getClass().getSimpleName() + ":" + System.lineSeparator() +
                "Cats: " + (cats.size() != 0 ? cats.stream().map(Cat::getName).collect(Collectors.joining(" ")) : "none") + System.lineSeparator() +
                "Toys: " + toys.size() + " Softness: " + toys.stream().mapToInt(Toy::getSoftness).sum();
    }
